...
namespace comet
    ...
    void Routes::registerAllHandlers()
      {
        std::vector<PossibleRoutes> allPossibleRoutes = {
             ...
             {true, false, "/servant/update_manager", Routes::ServantUpdateRemoteManager, {drogon::Post}},
  ...
  handleInvalidMethod(request);
    ...
    void Routes::ServantUpdateRemoteManager(const drogon::HttpRequestPtr &request, std::function<void(const drogon::HttpResponsePtr &)> &&callback)
    {
          handleInvalidMethod(request);
          ...
          try {
            ...
            resp->setStatusCode(k500InternalServerError);
            resp->setBody(std::string(R"({"ErrorMessage":"Exception occurred: )") + e.what() + R"("})");
            COMETLOG(std::string("Exception in /servant/update_manager: ") + e.what(), LoggerLevel::CRITICAL);
            callback(resp);
          }
     }
