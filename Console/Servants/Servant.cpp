...
namespace comet
{
	...
	void Servant::setCentralDataFolder(const std::string& newFolder)
	{
		centralDataFolder = newFolder;
	}

	std::string Servant::getCentralDataFolder()
	{
		return centralDataFolder;
	}
	...
	std::string Servant::htmlServantSummary(Database& db)
	{
		std::string html = "";
		...
		for (const auto& row : results) {
			...
			html += "<td>" + eFolder + "</td>";
			html += "<td>" + row.at("centralDataFolder") + "</td>";
			html += "</tr>";
			rowIndex++;
		}
		html += "</table>";
	...
	void Servant::updateDatabase(Database& db)
	{
		if (!db.insertRecord(
			...
			", code, port, totalCores, unusedCores, activeCores, managerIpAddress, engineFolder, centralDataFolder) "
			...
			", '" + managerIpAddress + "' , '" + engineFolder + "' , '" + centralDataFolder + "'  )", false
		)) {
