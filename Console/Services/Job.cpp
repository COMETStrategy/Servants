...
Job::Job(const std::string &emailHeader, const std::string &codeHeader, const Json::Value &json)
  {
    try {
      ...
      engineDirectory64 = json["attributes"]["EngineDirectory64"].asString();
      basePhaseDirectory64 = json["attributes"]["BasePhaseDirectory64"].asString();
      workingDirectory64 = json["attributes"]["WorkingDirectory64"].asString();
    ...
    comet::Logger::log("✅ " + jobStatusDescription() + " : " + title + " #" + caseNumber + " (" + caseName +") "); // Log the error message
  }
